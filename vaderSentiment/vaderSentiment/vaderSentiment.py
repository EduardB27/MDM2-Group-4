...
if __name__ == '__main__':
    ...
    while i_1!=len(df):
        hotels.append(df.iloc[i_1,0])
        i_1+=1
        # reviews is functionally useless
    while i_2!=len(df):
        reviews.append(df.iloc[i_2,1])
        i_2+=1
    ...
    for sentence in sentences:
        ...
        score.append(str(vs["compound"]))
        
        Final_L=[]
    while i_3!=len(df):
        Final_L.append([df.iloc[i_3,0],df.iloc[i_3,1],score[i_3]])
        i_3+=1
    x=len(score)
        
    sted=pd.DataFrame(Final_L,columns=['Hotel','Review','Score'])
    # input("\nPress Enter to continue the demo...\n")  # for DEMO purposes...
    sted.to_csv('Tier1_P_SA.csv')
   
# =============================================================================
# Necessary to run an input file into where sentences is. This should then
# work
# Next step:write a list of hotels and reviews to rewrite to a new file
# =============================================================================
